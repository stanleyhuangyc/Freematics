...
function updateUI() {
  ...
  s +=
    "</select><input type='button' onclick='goNextDash(previousSibling.value)' value='Switch'></input>";
...
var TRIPS = {
  ...
  loadTrip: function (tripIndex) {
    this.xhr.onreadystatechange = function () {
      ...
      var startTime = getDateFromGPSFormat(
        data.stats.start.date ? data.stats.start.date : tripDate,
        data.stats.start.time
      );
      var finishTime = getDateFromGPSFormat(
        data.stats.end.date ? data.stats.end.date : tripDate,
        data.stats.end.time
      );
      TRIPS.startTimeEpoc =
      ...
      features.push({
        type: 'Feature',
        properties: { name: 'Start', info: startTime },
        geometry: { type: 'Point', coordinates: data.trip.coordinates[0] },
      });
      features.push({
        type: 'Feature',
        properties: { name: 'Finish', info: finishTime },
        geometry: {
          type: 'Point',
          coordinates: data.trip.coordinates[length - 1],
        },
      });
      ...
      for (var i = 0; i < length; i++) {
        ...
        if (index == null && speed == 0) {
          ...
          var duration = Math.floor(
            (data.trip.timestamps[i] - data.trip.timestamps[index]) / 1000
          );
          ...
          if (duration >= STOP_TIME_MIN) {
            ...
            features.push({
              type: 'Feature',
              properties: { name: 'Stopping', info: duration + ' seconds' },
              geometry: {
                type: 'Point',
                coordinates: data.trip.coordinates[index],
              },
            });
          }
      ...
      var duration = Math.floor(
        (data.stats.end.ts - data.stats.start.ts) / 1000
      );
      ...
      info +=
        '<br/>Average Speed: ' +
        (((data.stats.distance / duration) * 3600) / 1000).toFixed(1) +
        ' km/h';
      if (totalStopping)
        info +=
          '<br/>Total Stopping Time: ' +
          (totalStopping / 60).toFixed(0) +
          ' minutes';
      ...
      summ +=
        '<br/><br/>Distance: ' +
        (data.stats.distance / 1000).toFixed(1) +
        ' km';
      summ += '<br/>Duration: ';
      if (hr) summ += hr + ':';
      summ += (min < 10 ? '0' : '') + min + ':';
      ...
      if (totalStopping)
        summ +=
          '<br/>Stopping: ' + (totalStopping / 60).toFixed(0) + ' minutes';
      summ +=
        '<br/>Average Speed: ' +
        (((data.stats.distance / duration) * 3600) / 1000).toFixed(1) +
        ' km/h';
      ...
      summ +=
        "<input type='button' value='Update' onclick='TRIPS.loadChart([parseInt(previousSibling.previousSibling.value),parseInt(previousSibling.value)])'/>";
      summ += '<hr/>';
      ...
      TRIPS.loadChart([
        parseInt(document.getElementById('chartPID0').value),
        parseInt(document.getElementById('chartPID1').value),
      ]);
    };
    ...
    html +=
      ...
      '&tripid=' +
      this.tripID +
      "' target='_blank'>RAW</a><hr/>";

    if (tripIndex < TRIPS.history.length - 1) {
      html +=
        "<input type='button' value='Previous' onclick='TRIPS.loadTrip(" +
        (tripIndex + 1) +
        ")'></input>";
    } else {
    ...
    if (tripIndex > 0) {
      html +=
        "<input type='button' value='Next' onclick='TRIPS.loadTrip(" +
        (tripIndex - 1) +
        ")'></input>";
    } else {
    ...
    html +=
      "<br/><br/><input type='button' onclick='TRIPS.showList()' value='Back'></input>";
  ...
  list: function () {
    this.xhr.onreadystatechange = function () {
      ...
      TRIPS.history.sort(function (a, b) {
        return b.key - a.key;
      });
      TRIPS.showList();
    };
    ...
    var url =
      ...
      USER.devid +
      '&begin=' +
      getISOFromLocal(parseInt(begin), 0) +
      '&end=' +
      getISOFromLocal(parseInt(end), 235959);
    //alert(url);
    this.xhr.open('GET', url, true);
    this.xhr.send(null);
  },
  showList: function () {
    ...
    for (var i = 0; i < this.history.length; i++) {
      ...
      html +=
        "<div class='link'><a onclick=\"TRIPS.loadTrip(" +
        i +
        ')">' +
        tm.substr(0, tm.indexOf(' ')) +
        ' (' +
        str +
        ')</a></div>';
    }
  ...
  showMarkerFromTime: function (t) {
    ...
    for (var i = 0; i < data.trip.timestamps.length; i++) {
      if (data.trip.timestamps[i] >= ts) {
        OSMAP.setMarker(0, [
          data.trip.coordinates[i][1],
          data.trip.coordinates[i][0],
        ]);
        var info =
          Highcharts.dateFormat('%Y-%m-%d %H:%M:%S', t) +
          '<br/>' +
          data.trip.speeds[i] +
          ' ' +
          data.trip.altitudes[i];
